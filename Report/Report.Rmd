...
To ensure a level playing field across models:

* We applied a **consistent preprocessing pipeline**—including feature generation via `make_features`, log-transformation of the target, and MinMax scaling of inputs—for all models.

* We used **chronological session-based splitting** based on `time_id` to preserve the temporal order and prevent forward-looking bias. An 80/10/10 train-validation-test split ensured strict separation between training and future data.

* All performance metrics were calculated **only on the held-out test sessions**, which were never seen during model training or validation.

This careful setup ensures that performance differences are due to the models themselves—not inconsistencies in data handling or leakage from future information.
