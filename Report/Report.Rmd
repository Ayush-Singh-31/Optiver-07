...
author: "Tobit Louis, Kylie Haryono, Christy Lee"
date: "2025-05-24"
format:
  html:
    code-tools: true
...
In evaluating the models, there were 3 metrics that are used for comparison: R-squared (R2), QLIKE loss, and Mean Squared Error (MSE). R-squared measures how much variance in the target is explained by the model, and a higher r-squared means better model fit. This makes it valuable for assessing general model accuracy. QLIKE loss is specifically designed for volatility forecasting, penalising inaccurate volatility forecasts, especially when predicted variances are too low. This matters in financial risk settings, such as in underestimating spikes. A lower QLIKE shows that it is more reliable in high-risk or volatile environments. MSE calculates the average squared difference between predicted and actual values. Its sensitivity to outliers helps capture how inaccurate a model might be on extreme values, making it useful for direct interpretation of raw prediction error, complementary to QLIKE.

R2 and MSE show how well a model fits, while QLIKE captures the financial impact of the errors. Together, these allow us to evaluate the general predictive power (R2), domain-specific volatility reliability (QLIKE), and outlier sensitivity (MSE).
...
In terms of accuracy, the transformer model outperformed all the other models. It achieved the lowest MSE and QLIKE, and the highest R2 on test data. This indicates a strong ability to capture complex, non-linear patterns in volatility.

Although LSTM had a slightly higher MSE than the Transformer, it still had a strong R2 and competitive QLIKE. It showed a higher level of consistency than Random Forest, indicating a better generalisation on sequential data. 

Random Forest performed better than WLS, but performance dropped due to the inability to model time-based patterns. This highlights the limits of static, tree-based models for time-series volatility. On the other hand, WLS was the weakest performer, but it established a useful baseline. It had the lowest R2 and higher QLIKE, showing that deep learning models deliver real gains over simple linear baselines, particularly on volatility data.

While Transformer delivered the highest predictive accuracy across all metrics, its complexity and opacity raise practical concerns for real-world trading deployment.
